...
int main() {
  ...
  // find number to divide by that won't affect divisibilities
  int divideBy = 1;
  for (int i = 0; i < monkeys.size(); i++) {
    divideBy *= monkeys.at(i).getDivisibleBy();
  }
  ...
  for (int r = 0; r < numRounds; r++) {
    for (int m = 0; m < monkeys.size(); m++) {
      ...
      for (int i = 0; i < curNumItems; i++) {
        ...
        // get worry level
        long worryLevel = monkeys.at(m).getItems().front();
        ...
        // manage worry level without affecting divisibilities
  ...
  long long mostInspected = 0;
  long long secondMostInspected = 0;
  for (int i = 0; i < monkeys.size(); i++) {
